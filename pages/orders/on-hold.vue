<template>
    ...
    <OrdersListProduction :list="onHoldList" 
...
<script setup lang="ts">
    ...
    // });
    const { data:onHold } = await useFetch('/api/orders/list/on-hold',{
        method:'GET'
    });
    ...
    const onHoldList = onHold?.value?.list?.length >0 ? onHold?.value?.list:[];
