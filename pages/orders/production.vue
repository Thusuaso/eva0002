<template>
    ...
    <OrdersListProduction :list="productionList" 
...
<script setup lang="ts">
    ...
    const productionList = production?.value?.list?.length >0 ? production?.value?.list:[];
    ...
    const newOrder = ()=>{
        ...
        ordersStore.setButtonSaveCheckDisabled(true);
        productList = ref([]);
    ...
    const saveOrderEmit = async (event:any)=>{
        if(event.MusteriID == 0 || event.MusteriID == undefined || event.MusteriID == '' || event.MusteriID == ' '){
            toast.add({severity:'error',detail:'Select your company name',life:3000});
            return;
        }else {
            if(event.SiparisSahibi == 0 || event.SiparisSahibi == undefined || event.SiparisSahibi == '' || event.SiparisSahibi == ' '){
                toast.add({severity:'error',detail:'Select your seller name',life:3000});
                return;
            }else{
                if(event.Operasyon == 0 || event.Operasyon == undefined || event.Operasyon == '' || event.Operasyon == ' '){
                    toast.add({severity:'error',detail:'Select your operation',life:3000});
                    return;
                }else{
                    if(event.Finansman == 0 || event.Finansman == undefined || event.Finansman == '' || event.Finansman == ' '){
                        toast.add({severity:'error',detail:'Select your finance manager',life:3000});
                        return;
                    }else{
                        if(event.TeslimTurID == 0 || event.TeslimTurID == undefined || event.TeslimTurID == '' || event.TeslimTurID == ' '){
                            toast.add({severity:'error',detail:'Select your shipped type',life:3000});
                            return;
                        }else{
                            if(event.OdemeTurID == 0 || event.OdemeTurID == undefined || event.OdemeTurID == '' || event.OdemeTurID == ' '){
                                toast.add({severity:'error',detail:'Select your payment type',life:3000});
                                return; 
                            }else{
                                if(event.UlkeId == 0 || event.UlkeId == undefined || event.UlkeId == '' || event.UlkeId == ' '){
                                    toast.add({severity:'error',detail:'Select your country',life:3000});
                                    return; 
                                }else{
                                    if(event.FaturaKesimTurID == 0 || event.FaturaKesimTurID == undefined || event.FaturaKesimTurID == '' || event.FaturaKesimTurID == ' '){
                                        toast.add({severity:'error',detail:'Select your invoice type',life:3000});
                                        return;
                                    }else{
                                        if(event.SiparisNo == '' || event.SiparisNo == undefined || event.SiparisNo == ' '){
                                             toast.add({severity:'error',detail:'Order number can not be empty',life:3000});
                                             return;
                                        }else{
                                            if(event.SiparisTarihi == '' || event.SiparisTarihi == undefined || event.SiparisTarihi == ' '){
                                                 toast.add({severity:'error',detail:'Order date can not be empty',life:3000});
                                                 return;
                                            }else{
                                                await ordersStore.setButtonCheckDisabled(true);
                                            const {data:order} = await useFetch('/api/orders/process/save',{method:'POST',body:event});
                                            if(order?.value?.status){
                                                await toast.add({severity:'success',detail:order?.value?.message,life:3000});
                                                await ordersStore.setButtonCheckDisabled(false);
                                                await ordersStore.setOrderId(order?.value?.id);
                                                ordersStore.setButtonSaveCheckDisabled(false);

                                            }else{
                                                await toast.add({severity:'error',detail:order?.value?.message,life:3000});
                                                await ordersStore.setButtonCheckDisabled(false);
                                            };
                                            }
                                        }
    ...
    const updateOrderEmit = async (event:any)=>{
        const data = {...event,'SiparisId':ordersStore.getOrderId}
        const { data:order } = await useFetch('/api/orders/process/update',{method:'PUT',body:data});
